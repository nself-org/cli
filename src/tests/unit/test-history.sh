#!/usr/bin/env bash
# test-history.sh - Tests for nself history
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/history.sh"
...
# Test: Command history documented
test_history_functionality() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -qi "command\|audit\|log"; then
    printf "✓ Command history functionality documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ History functionality not clear\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself history...\n"
test_syntax
test_help
test_history_functionality
