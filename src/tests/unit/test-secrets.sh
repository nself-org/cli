#!/usr/bin/env bash
# test-secrets.sh - Tests for nself secrets (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/secrets.sh"
...
test_new_command_mentioned() {
  ...
  if echo "$output" | grep -q "config secrets"; then
    printf "✓ Mentions 'nself config secrets'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself secrets (deprecated)...\n"
test_syntax
test_deprecation_warning
test_new_command_mentioned
