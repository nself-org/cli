#!/usr/bin/env bash
# test-rollback.sh - Tests for nself rollback (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/rollback.sh"
...
test_redirect() {
  ...
  if echo "$output" | grep -qi "rollback\|version\|previous"; then
    printf "✓ Redirects to 'nself backup rollback'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself rollback (deprecated)...\n"
test_syntax
test_deprecation_warning
test_redirect
