#!/usr/bin/env bash
# test-audit.sh - Tests for nself audit
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/audit.sh"
...
# Test: Shows audit trail functionality
test_audit_trail() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -qi "log\|trail\|event"; then
    printf "✓ Audit trail functionality documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Audit trail functionality not clear\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself audit...\n"
test_syntax
test_help
test_audit_trail
