#!/usr/bin/env bash
# test-restore.sh - Tests for nself restore (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/restore.sh"
...
test_redirect() {
  ...
  if echo "$output" | grep -qi "restore\|backup\|snapshot"; then
    printf "✓ Redirects to 'nself backup restore'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself restore (deprecated)...\n"
test_syntax
test_deprecation_warning
test_redirect
