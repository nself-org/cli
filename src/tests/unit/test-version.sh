#!/usr/bin/env bash
# test-version.sh - Tests for nself version
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/version.sh"
...
test_version_display() {
  ...
  if echo "$help_output" | grep -qi "version\|nself"; then
    printf "✓ Version display documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Version display not documented\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself version...\n"
test_syntax
test_help
test_version_display
