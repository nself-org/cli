#!/usr/bin/env bash
# test-billing.sh - Tests for nself billing (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/billing.sh"
...
test_deprecation_warning() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local output
  # Redirect will fail, but we can capture stderr which has the warning
...
test_new_command_mentioned() {
  ...
  if echo "$output" | grep -q "tenant billing"; then
    printf "✓ Mentions 'nself tenant billing'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself billing (deprecated)...\n"
test_syntax
test_deprecation_warning
test_new_command_mentioned
