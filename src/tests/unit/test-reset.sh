#!/usr/bin/env bash
# test-reset.sh - Tests for nself reset (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/reset.sh"
...
test_redirect() {
  ...
  if echo "$output" | grep -qi "reset\|remove\|factory\|initial"; then
    printf "✓ Redirects to 'nself backup reset'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself reset (deprecated)...\n"
test_syntax
test_deprecation_warning
test_redirect
