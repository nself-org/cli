#!/usr/bin/env bash
# test-status.sh - Tests for nself status
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/status.sh"
...
test_health_checking() {
  ...
  if echo "$help_output" | grep -qi "health\|service\|container"; then
    printf "✓ Service health checking documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Health checking not documented\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself status...\n"
test_syntax
test_help
test_health_checking
