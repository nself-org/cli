#!/usr/bin/env bash
# test-mfa.sh - Tests for nself mfa (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/mfa.sh"
...
test_new_command_mentioned() {
  ...
  if echo "$output" | grep -q "auth mfa"; then
    printf "✓ Mentions 'nself auth mfa'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself mfa (deprecated)...\n"
test_syntax
test_deprecation_warning
test_new_command_mentioned
