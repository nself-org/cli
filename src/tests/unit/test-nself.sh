#!/usr/bin/env bash
# test-nself.sh - Tests for nself main entry point
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/nself.sh"
...
test_command_listing() {
  ...
  if echo "$help_output" | grep -q "init\|build\|start\|stop"; then
    ...
    printf "✗ Main commands not listed\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself main...\n"
test_syntax
test_help
test_command_listing
