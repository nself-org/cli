#!/usr/bin/env bash
# test-metrics.sh - Tests for nself metrics
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/metrics.sh"
...
# Test: Metrics functionality documented
test_metrics_functionality() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -qi "performance\|monitor\|profil"; then
    printf "✓ Metrics functionality documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Metrics functionality not clear\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself metrics...\n"
test_syntax
test_help
test_metrics_functionality
