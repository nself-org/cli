#!/usr/bin/env bash
# test-server.sh - Tests for nself server (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/server.sh"
...
test_redirect() {
  ...
  if echo "$output" | grep -q "deploy server"; then
    printf "✓ Redirects to 'nself deploy server'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself server (deprecated)...\n"
test_syntax
test_deprecation_warning
test_redirect
