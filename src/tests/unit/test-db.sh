#!/usr/bin/env bash
# test-db.sh - Tests for nself db
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/db.sh"
...
# Test: Database subcommands documented
test_subcommands() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -q "migrate\|backup\|restore"; then
    printf "✓ Database subcommands documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Database subcommands missing\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself db...\n"
test_syntax
test_help
test_subcommands
