#!/usr/bin/env bash
# test-monitor.sh - Tests for nself monitor
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/monitor.sh"
...
# Test: Grafana integration mentioned
test_grafana_integration() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -qi "grafana\|dashboard\|prometheus"; then
    printf "✓ Monitoring stack integration documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Monitoring stack not documented\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself monitor...\n"
test_syntax
test_help
test_grafana_integration
