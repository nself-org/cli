#!/usr/bin/env bash
# test-destroy.sh - Tests for nself destroy
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/destroy.sh"
...
# Test: Destructive operation warning
test_warning_present() {
  ...
  if echo "$help_output" | grep -qi "warning\|danger\|permanent\|delete"; then
    ...
    printf "✗ Warning not documented\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself destroy...\n"
test_syntax
test_help
test_warning_present
