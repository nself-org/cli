#!/usr/bin/env bash
# test-checklist.sh - Tests for nself checklist
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/checklist.sh"
...
# Test: Checklist functionality documented
test_checklist_functionality() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -qi "check\|verify\|validate"; then
    printf "✓ Checklist functionality documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Checklist functionality not clear\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself checklist...\n"
test_syntax
test_help
test_checklist_functionality
