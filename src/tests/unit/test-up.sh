#!/usr/bin/env bash
# test-up.sh - Tests for nself up
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/up.sh"
...
# Test: Startup workflow documented
test_startup_workflow() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -qi "start\|build\|init"; then
    printf "✓ Startup workflow documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Startup workflow not documented\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself up...\n"
test_syntax
test_help
test_startup_workflow
