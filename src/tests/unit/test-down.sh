#!/usr/bin/env bash
# test-down.sh - Tests for nself down
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/down.sh"
...
# Test: Shutdown workflow documented
test_shutdown_workflow() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" --help 2>&1 || bash "$SOURCE_FILE" help 2>&1 || true)

  if echo "$help_output" | grep -qi "stop\|shutdown\|remove"; then
    printf "✓ Shutdown workflow documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Shutdown workflow not documented\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself down...\n"
test_syntax
test_help
test_shutdown_workflow
