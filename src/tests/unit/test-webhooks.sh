#!/usr/bin/env bash
# test-webhooks.sh - Tests for nself webhooks (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/webhooks.sh"
...
test_new_command_mentioned() {
  ...
  if echo "$output" | grep -q "auth webhooks"; then
    printf "✓ Mentions 'nself auth webhooks'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself webhooks (deprecated)...\n"
test_syntax
test_deprecation_warning
test_new_command_mentioned
