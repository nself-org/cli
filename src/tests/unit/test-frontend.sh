#!/usr/bin/env bash
# test-frontend.sh - Tests for nself frontend (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/frontend.sh"
...
test_redirect() {
  ...
  if echo "$output" | grep -qi "frontend\|deploy\|app"; then
    printf "✓ Redirects to 'nself dev frontend'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself frontend (deprecated)...\n"
test_syntax
test_deprecation_warning
test_redirect
