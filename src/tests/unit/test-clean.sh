#!/usr/bin/env bash
# test-clean.sh - Tests for nself clean (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/clean.sh"
...
test_redirect() {
  ...
  if echo "$output" | grep -qi "clean\|prune\|volume\|image"; then
    printf "✓ Redirects to 'nself backup clean'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself clean (deprecated)...\n"
test_syntax
test_deprecation_warning
test_redirect
