#!/usr/bin/env bash
# test-migrate.sh - Tests for nself migrate (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/migrate.sh"
...
test_redirect() {
  ...
  if echo "$output" | grep -qi "migrate\|migration\|vendor\|database"; then
    printf "✓ Redirects to 'nself perf migrate'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself migrate (deprecated)...\n"
test_syntax
test_deprecation_warning
test_redirect
