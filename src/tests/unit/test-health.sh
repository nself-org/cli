#!/usr/bin/env bash
# test-health.sh - Tests for nself health
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/health.sh"
...
# Test: Health check functionality documented
test_health_check() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  ...
  if echo "$help_output" | grep -qi "service\|container\|check"; then
    ...
    printf "✗ Health check functionality not clear\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself health...\n"
test_syntax
test_help
test_health_check
