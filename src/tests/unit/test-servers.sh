#!/usr/bin/env bash
# test-servers.sh - Tests for nself servers (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/servers.sh"
...
test_new_command_mentioned() {
  ...
  if echo "$output" | grep -q "deploy server"; then
    printf "✓ Mentions 'nself deploy server'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself servers (deprecated)...\n"
test_syntax
test_deprecation_warning
test_new_command_mentioned
