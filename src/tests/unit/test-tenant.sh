#!/usr/bin/env bash
# test-tenant.sh - Tests for nself tenant
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/tenant.sh"
...
# Test: Multi-tenancy subcommands
test_subcommands() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -q "create\|list\|delete\|billing\|org"; then
    printf "✓ Multi-tenancy subcommands documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Multi-tenancy subcommands missing\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself tenant...\n"
test_syntax
test_help
test_subcommands
