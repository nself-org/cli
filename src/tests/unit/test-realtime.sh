#!/usr/bin/env bash
# test-realtime.sh - Tests for nself realtime (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/realtime.sh"
...
test_new_command_mentioned() {
  ...
  if echo "$output" | grep -q "service realtime"; then
    printf "✓ Mentions 'nself service realtime'\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
...
# Run all tests
printf "Testing nself realtime (deprecated)...\n"
test_syntax
test_deprecation_warning
test_new_command_mentioned
