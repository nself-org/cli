#!/usr/bin/env bash
# test-exec.sh - Tests for nself exec
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/exec.sh"
...
# Test: Container execution documented
test_container_exec() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" help 2>&1)

  if echo "$help_output" | grep -qi "container\|command\|service"; then
    printf "✓ Container execution documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Container execution not clear\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself exec...\n"
test_syntax
test_help
test_container_exec
