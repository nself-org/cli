#!/usr/bin/env bash
# test-bench.sh - Tests for nself bench (deprecated wrapper)
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/bench.sh"
...
test_redirect() {
  ...
  if echo "$output" | grep -q "perf bench"; then
...
# Run all tests
printf "Testing nself bench (deprecated)...\n"
test_syntax
test_deprecation_warning
test_redirect
