#!/usr/bin/env bash
# test-restart.sh - Tests for nself restart
# Part of nself v0.9.9

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/../../cli/restart.sh"
...
# Test: Restart services functionality documented
test_functionality() {
  TESTS_RUN=$((TESTS_RUN + 1))
  local help_output
  help_output=$(bash "$SOURCE_FILE" --help 2>&1 || bash "$SOURCE_FILE" help 2>&1 || true)

  if echo "$help_output" | grep -qi "restart"; then
    printf "✓ Restart services functionality documented\n"
    TESTS_PASSED=$((TESTS_PASSED + 1))
    return 0
  else
    printf "✗ Restart services functionality not clear\n"
    return 1
  fi
}

# Run all tests
printf "Testing nself restart...\n"
test_syntax
test_help
test_functionality
