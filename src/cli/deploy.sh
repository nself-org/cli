...
deploy_environment() {
  ...
  deploy_script='cd '"'$deploy_path'"' 2>/dev/null || { echo "ERROR: Cannot access deploy path: '"$deploy_path"'" >&2; exit 1; }
    ...
    # Rebuild images and restart containers — always run from nself_build_dir
    # so docker compose picks up the docker-compose.yml generated by nself build
    cd '"'$nself_build_dir'"' 2>/dev/null || cd '"'$deploy_path'"' 2>/dev/null || true
